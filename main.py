...
from models.lobby import prompt_room_creation, prompt_room_join, prompt_seat_selection, wait_lobby_selection
...
async def main():
    ...
    message_scope = make_scope_name('room_messages', current_user.nick)
    countdown_scope = make_scope_name('global_countdown', current_user.nick)
    global_controls_scope = make_scope_name('global_controls', current_user.nick)
    ...
    def render_room_widgets(active_room: Optional[Room]):
        ...
        with use_scope(global_controls_scope, clear=True):
            put_buttons(
                [
                    {
                        'label': '刷新操作窗口',
        ...
        render_seat_panel(active_room, force=True)

    def clear_room_widgets():
        current_user.skill.pop('seat_panel_version', None)
        for scope in (message_scope, countdown_scope, global_controls_scope, seat_scope):
            try:
    ...
    while True:
        ...
        async def handle_seat_change(target_seat: Optional[int] = None, skip_prompt: bool = False):
            if room.started:
                toast('游戏已开始，暂不可调整座位', color='warn')
                return
            if target_seat is not None:
                try:
                    ...
                    target_seat = None
                if target_seat == current_user.seat:
                    return
            if current_user.skill.get('seat_changing'):
                return
            current_user.skill['seat_changing'] = True
            try:
                previous_seat = current_user.seat
                room.release_seat(current_user)
                current_user.skill['standing'] = True
                try:
                    if skip_prompt and target_seat is not None:
                        new_seat = target_seat
                    else:
                        new_seat = await prompt_seat_selection(room, current_user)
                finally:
                    current_user.skill.pop('standing', None)
                seat_choice = new_seat
                fallback_seat = previous_seat if previous_seat else None
                if seat_choice is None and fallback_seat:
                    if fallback_seat in room.list_available_seats():
                        seat_choice = fallback_seat
                try:
                    room.assign_seat(current_user, seat_choice)
                except ValueError:
                    valid_fallback = fallback_seat if fallback_seat and fallback_seat in room.list_available_seats() else None
                    room.assign_seat(current_user, valid_fallback)
                current_user.send_msg(f'你当前的号码牌：{current_user.seat}号')
            finally:
                current_user.skill.pop('seat_changing', None)

        def render_seat_panel(active_room: Optional[Room], force: bool = False):
            if not active_room:
                return
            snapshot = active_room.get_seat_snapshot()
            current_key = (snapshot['version'], active_room.started)
            last_version = current_user.skill.get('seat_panel_version')
            if not force and last_version == current_key:
                return
            current_user.skill['seat_panel_version'] = current_key
            with use_scope(seat_scope, clear=True):
                put_markdown('### 座位')
                ...
                if not seats:
                    ...
                    def seat_click(seat_no: int):
                        if active_room.started:
                            toast('游戏已开始，座位不可更换', color='warn')
                            return
                        asyncio.create_task(handle_seat_change(target_seat=seat_no, skip_prompt=True))

                    current_row = []
                    for info in seats:
                        seat_label = f"{info['seat']}号"
                        occupant = info.get('nick')
                        if occupant:
                            btn = put_button(
                                f'{seat_label}：{occupant}',
                                onclick=lambda: None,
                                color='secondary',
                                disabled=True
                            )
                            ...
                            if active_room.started:
                                ...
                                btn = put_button(
                                    f'{seat_label}：空',
                                    color='success',
                                    onclick=lambda sn=info['seat']: seat_click(sn)
                                )
                        ...
                        if len(current_row) == row_size:
                            ...
                            current_row = []
                    if current_row:
                        while len(current_row) < row_size:
                            current_row.append(put_text(''))
                        rows.append(current_row)
                    put_table(rows)
                standing = snapshot.get('standing') or []
                if standing:
                    put_text('未坐下：' + '、'.join(standing))

        render_room_widgets(room)
